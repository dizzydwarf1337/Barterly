...
public class LogServiceIntegrationTests : IClassFixture<DatabaseFixture>
{
    ...
    public async Task CreateLogWithProvidedData_ShouldCreateLog()
    {
        ...
        var savedLog =
            await _database.Logs.FirstOrDefaultAsync(x => string.Equals(x.Message.ToLower(), message.ToLower()));
    ...
    public async Task DeleteLogAsyncWithCreatedLog_ShouldDeleteLog()
    {
        ...
        var result = await _database.Logs.FirstOrDefaultAsync(x => x.Id == log.Id);
        // Assert
        Assert.Null(result);
    }
