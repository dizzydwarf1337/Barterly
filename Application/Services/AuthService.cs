...
public class AuthService : IAuthService
{
    ...
    public AuthService(UserManager<User> userManager, ITokenService tokenService, IMapper mapper,
        IConfiguration config, IUserFactory userFactory)
    {
    ...
    public async Task<LoginExternalResponse> LoginWithGmail(string code, CancellationToken cancToken)
    {
        var token = await GetGoogleAccessTokenAsync(code) ??
                    throw new ExternalServiceException("Failed to retrive google token");
        var IsFirstTime = false;
        var settings = new GoogleJsonWebSignature.ValidationSettings
        {
            Audience = new List<string>
                { _config["GoogleAPI:ClientId"] ?? throw new ConfigException("Google audience error") }
        };
        ...
        var tokenToSend = await _tokenService.GetLoginToken(user.Id, cancToken);
    ...
    private async Task<TokenResponse> GetGoogleAccessTokenAsync(string code)
    {
        ...
        var parameters = new Dictionary<string, string>
        {
            ...
            {
                "redirect_uri",
                _config["GoogleAPI:RedirectUri"] ?? throw new ConfigException("Google Redirect api error")
            },
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            var tokenResponse = JsonConvert.DeserializeObject<TokenResponse>(responseContent) ??
                                throw new ExternalServiceException("Google token null response");
            return tokenResponse;
        }

        throw new ExternalServiceException("Failed to get google access token");
    }
}