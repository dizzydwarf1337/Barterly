...
namespace Application.Features.Posts.Events.PostUpdatedEvent
{
    public class PostUpdatedEventHandler : INotificationHandler<PostUpdatedEvent>
    {
        ...
        public PostUpdatedEventHandler(IPostQueryRepository postQueryRepository, IUserQueryRepository userQueryRepository, IMailService mailService, INotificationCommandRepository notificationCommandRepository)
        {
        ...
        public async Task Handle(PostUpdatedEvent notification, CancellationToken cancellationToken)
        {
            ...
            var user = await _userQueryRepository.GetUserAsync((notification.UserId));
            var post = await _postQueryRepository.GetPostById((notification.PostId), (notification.UserId));
            ...
            var usNotification = new Notification { Message = "Your post: " + post.Title + " has been updated", Title = "Post updated", UserId = post.OwnerId};
