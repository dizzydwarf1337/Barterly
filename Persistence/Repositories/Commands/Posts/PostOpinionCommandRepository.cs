...
public class PostOpinionCommandRepository : BaseCommandRepository<BarterlyDbContext>, IPostOpinionCommandRepository
{
    ...
    public async Task<PostOpinion> CreatePostOpinionAsync(PostOpinion opinion, CancellationToken token)
    {
    ...
    public async Task DeletePostOpinionAsync(Guid id, CancellationToken token)
    {
    ...
    public async Task SetHiddenPostOpinionAsync(Guid id, bool value, CancellationToken token)
    {
        var opinion = await _context.PostOpinions.FindAsync(id) ?? throw new EntityNotFoundException("PostOpinion");
