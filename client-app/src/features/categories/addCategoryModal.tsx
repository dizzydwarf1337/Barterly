...
export default observer(function AddCategoryModal({open, onClose, category}: Props) {
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth>
            <Box display="flex" flexDirection="column" padding="20px" gap="20px"
                ...
                <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        <FormControl fullWidth>
                            ...
                            <Input id="nameEn-input" name="nameEN" onChange={(e) => handleCategoryChange(e)}
                                   value={categoryToAdd.nameEN} aria-describedby="nameEn-helper"
                        ...
                        <FormControl fullWidth>
                            ...
                            <Input id="namePl-input" name="namePL" onChange={(e) => handleCategoryChange(e)}
                                   value={categoryToAdd.namePL} aria-describedby="namePl-helper"
                                   sx={{color: "primary.contrastText"}}/>
                        ...
                        <FormControl fullWidth>
                            ...
                            <Input id="desc-input" name="description" onChange={(e) => handleCategoryChange(e)}
                                   value={categoryToAdd.description} sx={{color: "primary.contrastText"}}/>
                        </FormControl>
                    </Box>
                    <Box display="flex" justifyContent="flex-end" alignItems="center" gap="10px" width="100%">
                        ...
                        <IconButton onClick={() => setSubCategories(prev => [...prev, {
                    ...
                    <Box display="flex" flexDirection="column" gap="20px">
                        {subCategoriesToAdd.map((sub, index) => (
                            <Box key={sub.id} display="flex" flexDirection="row" justifyContent="center"
                                 alignItems="center" gap="20px">
                                <FormControl required>
                                    <InputLabel htmlFor={`subCategory-namePl-${index}`}
                                                sx={{color: "primary.contrastText"}}>Sub category {index + 1} name
                                        PL</InputLabel>
                                ...
                                <FormControl required sx={{color: "primary.contrastText"}}>
                                    <InputLabel sx={{color: "primary.contrastText"}}>Sub category {index + 1} name
                                        EN</InputLabel>
                                    <Input sx={{color: "primary.contrastText"}}
                    ...
                    <Box display="flex" flexDirection="row" justifyContent="space-between" width="100%">
                        <Button variant="contained" color="success" loading={categoryStore.categoryLoading}
                                onClick={() => handleSubmit()}>Submit</Button>
                        <Button variant="outlined" color="error" onClick={() => {
                            onClose();
                            setSubCategories([]), setError("");
                            setCategoryToAdd(({
                                id: uuid(),
                                nameEN: "",
                                namePL: "",
