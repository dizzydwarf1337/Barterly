        ...
        <Box width="100%" display="flex" flexDirection="column" gap="5px">
            <form onSubmit={handleLogin}>
                <Box sx={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    <TextField label="Email" onChange={handleLoginDataChange} type="email" name="Email" autoComplete="username" value={login.Email} required />
                    <TextField label="Password" onChange={handleLoginDataChange} type="password" name="Password" autoComplete="current-password" value={login.Password} required />
                    <Button variant="outlined" color="success" type="submit">
                        {userStore.userLoading || userStore.googleLoading ? <CircularProgress size="25px" /> : t("login")}
                    </Button>
                </Box>
            </form>
            <Divider sx={{ m: "10px" }}>
            ...
            <Box display="flex" justifyContent="center" alignItems="centet" width="100%" >
                <Button variant="contained" color="secondary" fullWidth onClick={() => { navigate("/sign-in");  uiStore.setIsMobileMenuOpen(false) }}>
