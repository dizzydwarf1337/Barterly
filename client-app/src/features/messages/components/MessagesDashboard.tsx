...
const MessagesDashboard = observer(({
  ...
  const chat = chatId ? messageStore.chats.find(c => c.id === chatId) : null;
    ...
    try {
      ...
      if(res.isSuccess){
        if (pageNum === 1) {
          ...
          chat.messages = [...res.value.items, ...chat.messages];
        }
      }
  ...
  const getOtherUserId = (): string => {
    ...
    return chat.user1.userId === authStore.user.id ? chat.user2.userId : chat.user1.userId;
  };
    ...
    try {
      ...
      if (isNewChat && !chatId) {
        messageStore.chats.push({
          id: newChatId,
          user1: { ...authStore.user, userId: authStore.user.id, imagePath: authStore.user.profilePicturePath },
          user2: { userId: receiver, ...otherUser, imagePath: otherUser?.profilePicturePath, firstName: otherUser?.firstName ?? '', lastName: otherUser?.lastName ?? ''},
          messages: [],
  ...
  const handleSendProposal = async () => {
    if (!proposalContent.trim() || !proposalPrice || !proposalPostId || !authStore.user) return;
    const receiver = isNewChat ? receiverId! : getOtherUserId();
      ...
      >
        ...
        {userLoading ? (
          ...
          <>
            <Avatar
              src={
                otherUser.profilePicturePath ?? undefined
              }
      ...
      >
        ...
        <DialogContent>
          <FormControl fullWidth margin="normal">
            ...
            <Select
              ...
              onChange={(e) => 
                setProposalPostId(e.target.value)
              }
        ...
        <DialogActions>
          ...
          <Button 
            ...
            variant="contained"
            disabled={!proposalContent.trim() || !proposalPrice || !proposalPostId}
          >
            {t('common.send')}
          </Button>
        </DialogActions>
